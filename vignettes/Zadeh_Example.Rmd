...
vignette: >
  %\VignetteIndexEntry{Solving Zadeh's Example}
...
#
devtools::load_all(".") # used while testing with R-devel
# attach package dst
# library(dst) When using the package in place of devtools
#
...
temp1 <- as.data.frame(cbind(rownames(ZExpert1$tt), ZExpert1$spec))
colnames(temp1)[1] <- "ZExpert1"
...
temp1 <- as.data.frame(cbind(rownames(ZExpert2$tt), ZExpert2$spec))
colnames(temp1)[1] <- "ZExpert2"
...
temp1 <- as.data.frame(cbind(rownames(e1$tt), e1$spec))
colnames(temp1)[1] <- "e1"
...
temp1 <- as.data.frame(cbind(rownames(e2$tt), e2$spec))
colnames(temp1)[1] <- "e2"
...
# show the relation
temp1 <- as.data.frame(cbind(rownames(r1$tt), r1$spec))
colnames(temp1)[1] <- "r1"
rownames(temp1) <- NULL
...
The calculations involved are following the principles of the valuation language of Shenoy ^[P. P. Shenoy. A Valuation-Based Language for Expert systems. lnternational Journal of Approxtmate Reasoning 1989, 3 383--411]; see also ^[P. P. Shenoy. Valuation-Based Systems. Third School on Belief Functions and Their Applications, Stella Plage, France. September 30, 2015]. The variables are linked to functions (called valuations). A function can be a piece of evidence attached to a variable or a relation between two or more variables.
...
temp1 <- as.data.frame(cbind(rownames(Expert1_ext$tt), Expert1_ext$spec))
colnames(temp1)[1] <- "Expert1_ext"
...
temp1 <- as.data.frame(cbind(rownames(r2$tt), r2$spec))
colnames(temp1)[1] <- "r2"
...
temp1 <- as.data.frame(cbind(rownames(rel_2$tt), rel_2$spec))
colnames(temp1)[1] <- "rel_2"
