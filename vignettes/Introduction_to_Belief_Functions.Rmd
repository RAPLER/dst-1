...
specrwt <-  matrix(c(1,1,1,2,
                     1,1,1,0), ncol = 2, dimnames = list(NULL, c("specnb", "mass"))) 
# The relation
noW_if_R <- bcaRel(tt = ttrwt, spec = specrwt, infovar = inforwt, varnames = c("RdWorks", "Rain"), relnb = 1)
 noW_if_R
```

Suppose I didn't see Rdwork.  We now change the distribution of the bcaspec *rdworks* in order to reflect this condition.
```{r}
# Evidence of rain on the day of departure
rdworks$spec[,2] <- c(1, 0, 0)
bcaPrint(rdworks)
```
Finally, I would like to combine this piece of evidence with the relation between Rain and roadworks in order to evaluate the chances of raining on the day of my trip. I proceed in three steps to achieve that:
Firstly extend the Evidence (variable rdworks) to the product space Rain x Roadworks.
...
rain_xtnd <- extmin(rdworks, noW_if_R)
bcaPrint(rain_xtnd)
```
