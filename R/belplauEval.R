...
#' @param strict=TRUE whether true_order distinguish elements > 0
...
belplauEval<-function(bel_plau,true_order,var="rplau",err="type I",is_belplau=TRUE,strict=TRUE) {
  ...
  if (err=="type I") {
    type_i<-validated[true_order>0,if(strict) (true_order==0)]
    type_i_v<-as.vector(type_i)
    type_i_err<-1-sum(type_i_v==0)/length(type_i_v)
    return(type_i_err)
  } else if (err=="type II") {
    type_ii<-validated[true_order==0,if(strict) (true_order>0)]
