...
dsrwon<-function(x, y, sparseM = FALSE, mcores = "no", varnames = NULL, relnb = NULL, infovarnames) {
  #
 ...
 #  if (is.null(zx$ssnames) | is.null(zy$ssnames) ) {
    z <- list(con = con, tt=tt, spec = spec, infovar = infovar, varnames = varnames, valuenames = valuenames, inforel = inforel, I12=I12, sort_order=1:nrow(tt), ssnames = NULL, sfod = NULL)
  class(z) <- append(class(z), "bcaspec") 
  } 
  else {
    z <- list(con = con, tt = tt, spec = spec, infovar = infovar, varnames = varnames, valuenames = valuenames, inforel = inforel, I12=I12, sort_order=1:nrow(tt), ssnames = W1_list[sort_order], sfod = zx$sfod)
    class(z) <- append(class(z), "bcaspec") 
  }
  return(z)
  }
