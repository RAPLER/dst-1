...
bcaPrintL <- function(x, num_cut=10, num_top_mass=10) {
  # Local variables: conf, labs, size, m, df, df_by_size, df_by_m
  conf <- x$con
  labs <- x$ssnames
  size <- unlist(lapply(labs, length))
  m <- x$spec[, 2]
  df <- data.frame(m=m, size=size)
  df_by_size <- df %>% mutate(size_bins = cut_number(size, n = num_cut)) %>% group_by(size_bins)
  df_by_m <- df %>% mutate(m_bins = cut_number(m, n = num_cut)) %>% group_by(m_bins)
  ...
  print(paste("top",num_top_mass,"masses :"))
  print(sort(m, decreasing = TRUE)[1:num_top_mass])
