...
test_that("bcaspec_1", {
 ...
 result <- bca(f, m, cnames, infovarnames = c("burglary", "boom") )
  ...
  # T6: take names from "infovaluenames" parameter only if it is a string. If the matrix f is defined on a product space, the names must be the column names of the matrix. 
  ...
  infovarnames = c("RdWorks", "Rain")
  valuenames <- list(Rain= c("Ry", "Rn"), RdWorks=c("rWdy", "rWdn") )
  result <- bca(f = tt1, m, cnames, infovar = infovar, infovarnames = infovarnames, infovaluenames = valuenames)
