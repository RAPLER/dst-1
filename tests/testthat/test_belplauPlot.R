...
test_that("belplauPlot", {
  ...
  bel_plau_plot <- belplauPlot(matrix(rep(1,101),ncol=1,dimnames=list(NULL,"rplau")), 1:101, c(rep("B",50),"A",rep("B",50)), levels=c("A","B"),is_log_scale=TRUE,is_negative = TRUE)
  expect_equal(max(bel_plau_plot$data$rplau),0)
})
